...
!(function ($) {
  ...
  Datepicker.prototype = {
    ...
    place: function () {
      ...
      var rightSpace = viewportWidth - offset.left - pickerWidth;
      var gap = 34; // 30px gap

      // Check if there's enough space to place the picker below or above
      if (bottomSpace < pickerHeight + gap) {
        // Place above if there's not enough space below, with a 30px gap
        this.picker.css({
          top: offset.top - pickerHeight - gap,
          left: offset.left,
          display: "block",
        });
      } else {
        // Place below by default, with a 30px gap
        this.picker.css({
      ...
      // Adjust horizontal placement based on available space
      if (leftSpace < 0) {
        // Adjust if not enough space on the left
        this.picker.css({
          left: 0,
        });
